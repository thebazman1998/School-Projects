...
def descLive(x):
    ...
    if x == 0:
        ...
        print "There is a torn-up couch, a television across it,\na remote right beside the television, and a locked doorn"
...
def playLive():
    ...
    while key == 0 and endLevel == 0: #While user does not have the key, and level is not finished
        ...
        elif command == "use remote, couch" or command == "use remote, door":
            print "\nThe remote cannot control that!\n"
    ...
    while endLevel == 0: # While the level is finished
        thirdRoom(1)

def thirdRoom(x):
    methods.key2continue()
    introHall(x)
    playHall()
