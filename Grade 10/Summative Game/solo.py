        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                    ...
                    if select == location1:
                        return ["1",63,8,"Back In Black",171.5]
                    if select == location2:
                        return ["2",63,8,"About a Girl",0.0]
                    if select == location3:
                        return ["3",60,4,"About a Girl",0.0]
                    break
                elif event.key == pygame.K_F1:
                    main()
...
def play(song, diff, bpm, maxBeat, name, start):
    ...
    s = open("Audio/solo"+str(song)+"_"+str(diff)+".txt","r")
    ...
    pygame.display.set_caption("Playing "+name)
    ...
    while True:
        ...
        if barCount < len(bar):
            ...
            songEnd(score,highest,song,notesHit,notes)
        ...
        if newScore > 0:
            if newScore == 50:
                notesHit += 1
                if note[0] == 400:
                    screen.blit(hit1,box_k1)
                if note[0] == 500:
                    screen.blit(hit2,box_k2)
                if note[0] == 600:
                    screen.blit(hit3,box_k3)
                if note[0] == 700:
                    screen.blit(hit4,box_k4)
                if note[0] == 800:
                    screen.blit(hit5,box_k5)
                score += 50
                if noteStreak > 9:
                    score += 50
                    if noteStreak > 19:
...
def songEnd(score,highest,song,notesHit,notes):
    pygame.quit()
    sys.exit(0)
...
def main():
    diffSong = diffSelect()
    song = songSelect()
    numSong, bpm, maxBeat, nameSong, startSong = song[0], song[1], song[2], song[3], song[4]
    play(numSong,diffSong,bpm,maxBeat,nameSong,startSong)
