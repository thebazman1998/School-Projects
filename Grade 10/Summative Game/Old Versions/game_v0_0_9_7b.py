...
def play(song, diff, bpm, maxBeat, name, speed):
    ...
    while True:
        ...
        if barCount < len(bar):
            ...
            songEnd(score,highest,name,notesHit,notes)
...
def songEnd(score,highest,name,notesHit,notes):

    notePercent = round((notesHit/notes)*100,1)
    img = pygame.image.load("Graphics/End Screen.jpg")
    fontTitle = pygame.font.Font("Graphics/28 Days Later.ttf", 30)
    fontOptions = pygame.font.Font("Graphics/setback.ttf", 30)
    ...
    while not(done):
        ...
        location1 = (780,300,250,80)
        location2 = (780,400,250,80)
        
        pygame.draw.rect(screen,(0,g,0),select)

        screen.blit(songTitle,title)
        screen.blit(backMenu,location1)
        screen.blit(restart,location2)
